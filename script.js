...
function calculateSpeed(lon1, lat1, lon2, lat2, dtSeconds) {
  if (!dtSeconds || dtSeconds <= 0) return null;
  const distance = diagonal(lon1, lat1, lon2, lat2);
  return distance / dtSeconds;
}

const SpeedCounter = (function () {
  const lastPositions = new Map();
  function updateAndGet(satelliteId, lon, lat, alt, timeMs = Date.now()) {
    const prev = lastPositions.get(satelliteId);
    ...
    if (prev) {
      ...
      if (dt > 0) {
        distance = diagonal(prev.lon, prev.lat, lon, lat);
        speed = distance / dt;
      }
    }
    lastPositions.set(satelliteId, { lon, lat, alt, time: now });
    return { speed, distance, dt };
  }
  function getLast(satelliteId) {
    return lastPositions.get(satelliteId) || null;
  }
  function reset() {
    lastPositions.clear();
  }
  return { updateAndGet, getLast, reset };
})();
  ...
  for (let id = 1; id <= maxId; id++) {
    try {
      ...
      if (geo?.coordinates?.length === 3) {
        ...
        const speedInfo = SpeedCounter.updateAndGet(
          data.id,
          lon,
          lat,
          alt,
        ...
        const speedDisplay =
          speedInfo.speed !== null && speedInfo.speed !== undefined
            ? `${speedInfo.speed.toFixed(2)} m/s`
            : "brak danych";
        const modesText =
          (props.modes || []).map((m) => m.name).join(", ") || "brak";
        container.innerHTML += `<a href='#'> <div class='satelita'> <h3>${
          props.name || "brak nazwy"
        }</h3> <strong>ID:</strong> ${
          data.id
          ...
          props.norad_id ?? "brak"
        }<br> <strong>Tryby:</strong> ${modesText}<br> <strong>Współrzędne:</strong> <br>D: ${lon}<br> S: ${lat}<br> W: ${alt}<br> <strong>Prędkość</strong>: ${speedDisplay}</div> </a>`;
        ...
        container.innerHTML += `<a href='#'> <div class='satelita'> <h3>${
          ...
          props.norad_id ?? "brak"
        }<br> <strong>Tryby:</strong> ${modesText}<br> <strong>Współrzędne:</strong> brak danych<br> </div> </a> `;
      }
    } catch (err) {
...
const odswiezBtn = document.getElementById("odswiez");
if (odswiezBtn)
  odswiezBtn.onclick = function () {
    fetchSatellites();
  };
